...
const WithFallbackData = ({ children }: { children: React.ReactNode }) => (
  <SWRConfig value={{ fallbackData }}>{children}</SWRConfig>
)

describe('usePrefectures', () => {
  it('should return prefectures', async () => {
    const { result } = renderHook(() => usePrefectures(), {
      wrapper: WithFallbackData,
    })

    await waitFor(() => {
      expect(result.current.prefs).toHaveLength(47)
      expect(result.current.prefs?.[0].prefCode).toBeTypeOf('number')
      expect(result.current.prefs?.[0].prefName).toBeTypeOf('string')
    })
  })
})
